...
in
  stdenv.mkDerivation {
    ...
    buildCmd = ''
      ...
      # `batch-update-autoloads' comes from the obsolete autoload.el, whose file
      # regexp is derived from `get-load-suffixes' and only drops .elc/.so/.dll.
      # A native module named *.dylib placed in the package root is therefore
      # scanned as Lisp and aborts the build on Darwin. `loaddefs-generate'
      # drops .dylib too, so use it instead. EXTRA-DATA is passed to keep the
      # output file generated even for packages without autoload cookies, which
      # is what batch-update-autoloads did.
      rm -f "${ename}-autoloads.el"
      emacs --batch -l loaddefs-gen \
          --eval "(setq backup-inhibited t)" \
          --eval "(setq version-control 'never)" \
          --eval "(loaddefs-generate \".\" \"$PWD/${ename}-autoloads.el\" nil \";;; Generated by twist\n\")"
    '';
