...
let
      ...
      lib.pipe elpaEntries [
        ...
        (lib.mapAttrs (
          _: { core, ... }: {
            ...
            src = args.core-src;
            doTangle = true;
            files = fileListToAttrs (if isString core then [ core ] else core);
          }
        ))
      ]
    else
  ...
  baseNameFromString =
    pathString:
    if match baseNameRegexp pathString != null then
      head (match baseNameRegexp pathString)
    else
      pathString;
  ...
  makeExternal =
    ...
    let
      ...
      p =
        if entry ? ignored-files then
          file: all (pattern: match pattern file == null) ignorePatterns
        else
          _: true;
    in
    self:
    {
      doTangle = true;
      src =
        if hasAttr ename flakeLockData then
          fetchTree flakeLockData.${ename}
        else
      ...
      files =
        lib.pipe
          (
            ((if entry ? lisp-dir then filter (file: !isElisp file) else lib.id) (filesInDir self.src ""))
